...
async def youtube_to_notes(body: YouTubeRequest, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # Fallback to YouTube oEmbed API if yt-dlp is blocked
        try:
            import httpx
            import urllib.parse
            oembed_url = f"https://www.youtube.com/oembed?url={urllib.parse.quote(body.url)}&format=json"
            with httpx.Client() as client:
                resp = client.get(oembed_url, timeout=5.0)
                resp.raise_for_status()
                oembed_data = resp.json()
                title = oembed_data.get("title", "YouTube Video")
                ch = oembed_data.get("author_name", "")
                desc = "No description available (Fallback mode)."
                dur = ""
                tags = ""
        except Exception as e2:
            raise HTTPException(400, f"YouTube Extraction Error: {str(e)}")
